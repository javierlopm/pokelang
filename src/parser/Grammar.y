...
Ins : {- λ -}                 {% return (TypeVoid, newBlock ) }
    | Ins Exp "="  Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] TypeVoid TypeVoid >>= checkOkIns (Assign    (sel3 $2) (sel3 $4)) (snd $1)  } 
    | Ins Exp "*=" Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] TypeVoid TypeVoid >>= checkOkIns (AssignMul (sel3 $2) (sel3 $4)) (snd $1)  } 
    | Ins Exp "+=" Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] TypeVoid TypeVoid >>= checkOkIns (AssignSum (sel3 $2) (sel3 $4)) (snd $1)  } 
    | Ins Exp "-=" Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] TypeVoid TypeVoid >>= checkOkIns (AssignMin (sel3 $2) (sel3 $4)) (snd $1)  } 
    ...
    | Ins IF    Exp ":" Ent0 SmplDcls Ins Ent1 NextIf Else END     {% checkAllOk [(checkGuarded $2 $3 $7), (return (fst $10)), (return (fst $1))] TypeVoid TypeVoid >>= checkOkIns (mergeIf (Guard (trd $3) (snd $7)) (snd $9) (snd $10) ) (snd $1) } --{% checkOkIns (addToBlock (mergeIf (Guard ExpTrue) $9 $10 )) $1 } -- verificar que $3 es bool, $9 y $10 son void
    | Ins WHILE Exp ":" Ent0 SmplDcls Ins Ent1 END                 {% checkAllOk [(checkGuarded $2 $3 $7), (return (fst $1))] TypeVoid TypeVoid                     >>= checkOkIns (While (trd $3) (snd $7) ) (snd $1) }               --{% checkOkIns (addToBlock (While ExpTrue) ) $1  }
    -- left it here
    | Ins FOR Ent3 "=" Exp  "|" Exp "|" Exp ":"  SmplDcls Ins  END {% exitScope >> checkAllOk [ checkFor     $2   [$5,$7,$9] , return (fst $12), return (fst $1) ] TypeVoid TypeVoid >>= checkOkIns (ForStep (trd $5) (trd $7) (trd $9) (snd $12)) (snd $1) } -- MISSING INSTRUCTIONS
    | Ins FOR Ent3 "=" Exp  "|" Exp         ":"  SmplDcls Ins  END {% exitScope >> checkAllOk [ checkFor     $2   [$5,$7]    , return (fst $10), return (fst $1) ] TypeVoid TypeVoid >>= checkOkIns (For (trd $5) (trd $7) (snd $10)) (snd $1) } 
    | Ins FOR Ent4 "=" ENUM "|" ENUM        ":"  SmplDcls Ins  END {% exitScope >> checkAllOk [ checkEnumFor $2 $3 $5 $7     , return (fst $10), return (fst $1) ] TypeVoid TypeVoid >>= checkOkIns (For ((ExpEnum .lexeme) $5) ((ExpEnum .lexeme) $7) (snd $10)) (snd $1) }


-- List of elseif
NextIf: {- λ -}                                     {% return (TypeVoid,newIf)  } 
      | NextIf ELIF  Exp ":" Ent0 SmplDcls Ins Ent1 {% checkAllOk [(checkGuarded $2 $3 $7),(return (fst $1))] >>= checkAndBuild (insertIf (snd $1) (Guard (trd $3) (snd $7))) }
