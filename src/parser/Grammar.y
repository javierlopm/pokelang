...
Ins : {- λ -}                 {% return (TypeVoid, newBlock ) }
    | Ins Exp "="  Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] (sel1 $2) (sel1 $4) (lexeme (sel2 $4)) $3 0 >>= checkOkIns (Assign    (sel3 $2) (sel3 $4)) (snd $1)  } 
    | Ins Exp "*=" Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] (sel1 $2) (sel1 $4) (lexeme (sel2 $4)) $3 1 >>= checkOkIns (AssignMul (sel3 $2) (sel3 $4)) (snd $1)  } 
    | Ins Exp "+=" Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] (sel1 $2) (sel1 $4) (lexeme (sel2 $4)) $3 2 >>= checkOkIns (AssignSum (sel3 $2) (sel3 $4)) (snd $1)  } 
    | Ins Exp "-=" Exp   ";"  {% checkAllOk [checkLValue (sel1 $2, sel2 $2), (return (fst $1))] (sel1 $2) (sel1 $4) (lexeme (sel2 $4)) $3 3 >>= checkOkIns (AssignMin (sel3 $2) (sel3 $4)) (snd $1)  } 
    ...
    | Ins PRINT "("  STRING   ")" ";" {% checkOkIns (buildPrint $4   ) (snd $1) (fst $1) } --revisar
    | Ins READ  "("  ID       ")" ";" {% checkOkIns (buildRead  $4 $1) (snd $1) (fst $1) } --revisar
...
NextIf: {- λ -}                                     {% return (TypeVoid,newIf)  } 
      | NextIf ELIF  Exp ":" Ent0 SmplDcls Ins Ent1 {% checkAllOk [(checkGuarded $2 $3 $7),(return (fst $1))] TypeVoid TypeVoid "" $2 0 >>= checkAndBuild (insertIf (snd $1) (Guard (trd $3) (snd $7))) }
...
Exp :  -- Cambiar los NoExp por las Exp
    ...
    | Exp "^"  Exp      {% checkBinary [TypeInt] (sel1 $1) (sel1 $3) $2 (sel2 $1) >>= expIns (Binary Power (sel3 $1) (sel3 $3)) }   -- Int  and Int
    ...
    | Exp "//" Exp      {% checkFloatDiv [] (sel1 $1) (sel1 $3) $2 (sel2 $1) >>= expIns (Binary FloatDiv (sel3 $1) (sel3 $3)) }   -- last one integer
    ...
    --Direccion de variable
    | "&" Exp %prec AMP    {% return (TypeInt,(sel2 $2),Unary Address (sel3 $2))  } -- return $ if (isError (sel1 $2)) then (TypeError,sel2 $2,NoExp) else (TypePointer(sel1 $2),sel2 $2,(Unary Access (sel3 $2)))
    -- Asociatividad.
    | "(" Exp ")"    {% return $2 }
    -- Constantes.
    -- Llamadas
